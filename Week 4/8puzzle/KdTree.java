...
public class KdTree {
    ...
    /* inner node of teh 2d- tree */
    private static class KdNode
    {
        private Point2D p; // point created from the node
        private RectHV rect;  // outer rectangle surrounding the node
        private KdNode left; // left is also down
        private KdNode right; // the right is also up
       // private boolean isVerticalLineNode;  // if is vertical, then compare x-coordinates
        private KdNode (Point2D point , RectHV rectRange)
        {
            this.p = point;
            this.rect = rectRange;
        }
   ...
   /* Inserts the point p into 2d- tree */
   public void insert(Point2D p)              // add the point to the set (if it is not already in the set)
   {
       // last param is true if vertival line
       root = insert(root, p,0.0, 0.0, 1.0, 1.0, true);
       
   }
   
   /* returns true if the 2d- tree contains the input p, false otherwose*/
    ...
    *  until you reach an empty node and insert there. Keep track of parent 
    ...
    *  xcmp : true if the current node is vertical (requires x - val comparision)
    */
   
   private KdNode insert(KdNode node, Point2D p, double x0, double y0, double x1, double y1, boolean xcmp )
   {
       // insert when you reach at empty location
       if (node == null)
       {
           size ++;
       ...
       // if the point already exists, jst return
       else if (node.p.x() == p.x() && node.p.y() == p.y()) return node;
       
       // if current node is vertical, compare x coordinates
       if (xcmp)
       {
     ...
     * xcmp: true if the current node is vertical (requires x-val comparison) */
     ...
     {
         // false if you didnt find it
         if (node == null) return false;
         else if (node.p.x() == p.x() && node.p.y() == p.y()) return true;
         else
         {
             // current node is vertical : compare x-co-ordinates
             if ( xcmp )
             {
                 double cmp = p.x() - node.p.x();
     ...
     * drawVert: true if current node is vertical (need to draw vertical line) */
     private void draw(KdNode node, boolean drawVert)
     {
         ...
         // draw vertical line with x-coordinate of point and y-coordinate of parent rectangle
         if (drawVert)
         {
     ...
     * q:    Queue containing the points inside the rectangle */
     private void range(KdNode node, RectHV rect, Queue<Point2D> q)
     {
         ...
         if (rect.contains(node.p))
             q.enqueue(node.p);
         // check the left and right sub trees if the input rectangle intersects the
         // current rectangle
   ...
   public static void main(String[] args)                  // unit testing of the methods (optional) 
   {
       ...
       for (int i = 0; i< 1000 ; i++)
       {
           ...
           tree.insert(point);
           out.println(tree.contains(point)+ " ");
       }
       out.println();
       out.println("Size = "+ tree.size());
       out.println("Is Contains(0.1, 0.1) = "+ tree.contains(new Point2D(0.1,0.1)));
       //tree.drawRect(0.45,0.45,0.55,0.55);
       tree.draw();
       
       RectHV rangeRect = new RectHV(0.45,0.45,0.55,0.55);
      for (Point2D inRangePoint: tree.range(rangeRect)) {
         System.out.println(inRangePoint.toString());
      }
      System.out.println("Nearest point is " + tree.nearest(new Point2D(0.5, 0.5)).toString());
